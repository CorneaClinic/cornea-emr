...
(function (global) {
  ...
  async function saveNewEpisode() {
    ...
    if (!patientName || !diagnosis || !plannedProcedure) return;
  ...
  function openPreopModal() {
    if (_currentEpisode) fillPreopModal(_currentEpisode);
  }
  ...
  function openSafetyModal() {
    if (_currentEpisode) renderSafetyForm(_currentEpisode);
  }
  ...
  async function applyOverride() {
    ...
    if (!id || !reason) {
      setOfflineBanner('Override reason is required.');
      return;
    }
  ...
  function openScheduleModal() {
    if (_currentEpisode) fillScheduleModal(_currentEpisode);
  }
