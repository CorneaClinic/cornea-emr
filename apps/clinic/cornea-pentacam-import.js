/**
 * Pentacam CSV import — parse chamber.csv, BAD.CSV, and common export variants.
 * Project 4: maps to KC registry topography + laser refractive work-up fields.
 */
(function (global) {
  'use strict';
  ...
  const FIELD_ALIASES = {
    ...
    capturedAt: ['date', 'exam_date', 'examination_date', 'scan_date', 'datetime', 'examtime', 'time', 'datum', 'untersuchungsdatum'],
    kmax: ['kmax', 'k_max', 'k_max_front_d', 'max_k', 'maximum_k', 'kmax_front', 'kmax_ant', 'simk_max'],
    kmean: ['kmean', 'km', 'k_mean', 'simk2', 'average_k', 'mean_k', 'simk_mean', 'kmean_front'],
    k1: ['k1', 'simk_flat', 'flattest_k', 'k1_front', 'rf', 'simk1', 'k_flat'],
    k2: ['k2', 'simk_steep', 'steepest_k', 'k2_front', 'rs', 'simk2_steep', 'k_steep'],
    thinnestPachy: ['pachy_min', 'pachymin', 'thinnest_pachymetry', 'min_pachy', 'pachymetry_min', 'thinnest', 'min_thickness', 'dt'],
    centralPachy: ['pachy_apex', 'central_pachymetry', 'pachy_central', 'apex_pachy', 'pachymetry', 'pachy_mid'],
    badD: ['bad_d', 'badd', 'final_d', 'dfinal', 'd_final', 'bad', 'belin_d', 'deviation_index'],
    abcd: ['abcd', 'abcd_classification', 'abcd_grade', 'abcd_class'],
    anteriorElevation: ['ele_f_tp', 'anterior_elevation', 'front_elevation', 'elevation_front'],
    posteriorElevation: ['ele_b_tp', 'posterior_elevation', 'back_elevation', 'elevation_back'],
    coneLocation: ['cone_location', 'kmax_location', 'apex_location'],
  ...
  function looksLikeHeader(cells) {
    ...
    return /kmax|k_max|pachy|bad|eye|simk|patient|date|exam/.test(joined);
  }
  ...
  function mapObjectToReading(obj, forcedEye) {
    ...
    if (!eye) return null;
    if (mapped.kmax == null && mapped.thinnestPachy == null && mapped.badD == null && mapped.kmean == null) {
      return null;
    }
  ...
  function parsePentacamCsv(text) {
    ...
    if (!lines.length) {
      return { readings: [], warnings: ['File is empty'], format: 'unknown' };
    }
  ...
  function dedupeReadings(readings) {
    ...
    return readings.filter((r) => {
      const key = `${r.eye}|${r.capturedAt}|${r.kmax}|${r.thinnestPachy}`;
  ...
  function toKcTopoRow(reading, kcPatientId) {
    return {
      ...
      kcTopoNotes: reading.source === 'pentacam_csv' ? 'Imported from Pentacam CSV' : '',
      source: 'pentacam_csv'
    };
  }

  function toLaserWorkupPatches(readings) {
    const patches = { topography: { device: 'Pentacam', od: {}, os: {}, shared: {} }, corneal: { od: {}, os: {} } };
