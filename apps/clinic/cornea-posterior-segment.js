/**
 * Cornea Clinic — Posterior Segment / Fundus Examination Builder (Project 7)
 * Syncs structured state ↔ legacy fields (lidRE, corneaLE, …) for backward compatibility.
 */
(function (global) {
  'use strict';

  const T = global.CorneaPosteriorSegmentTaxonomy;
  if (!T) return;

  const ROOT_ID = 'posteriorSegmentBuilder';
  const JSON_FIELD = 'posteriorSegmentJSON';
  ...
  function checkConflicts(structureId, eye) {
    ...
    if (structureId === 'foveal' && eyeState.status === 'normal' && ids.includes('foveal_absent')) {
      warnings.push('Foveal reflex cannot be Present with absent foveal reflex finding.');
    }
  ...
  function applyTemplate(template) {
    const t = T.CLINICAL_TEMPLATES?.[template];
    if (!t) return;
    (t.findings || []).forEach((f) => {
      const structId = f.structure;
      T.EYES.forEach((eye) => {
  ...
  function pullPreviousVisit() {
    if (typeof global.pullPreviousFundusExam === 'function') {
      global.pullPreviousFundusExam();
      setTimeout(() => {
  ...
  function renderToolbar() {
    return `
      <div class="asb-toolbar no-print" role="toolbar" aria-label="Posterior segment tools">
        ...
        <div class="asb-toolbar-row">
          <input type="search" class="asb-search" id="psbSearchInput" placeholder="Search findings… ( / )" value="${escapeHtml(searchQuery)}" aria-label="Search findings">
          <div class="asb-mode-tabs">
            <button type="button" class="asb-mode-btn${activeMode === 'clinical' ? ' active' : ''}" data-asb-mode="clinical">Clinical templates</button>
          </div>
        </div>
  ...
  function renderModePanel() {
    const map = T.CLINICAL_TEMPLATES || {};
    const btns = Object.entries(map).map(([key, t]) =>
      `<button type="button" class="asb-quick-btn" data-asb-template="${escapeHtml(key)}" data-asb-mode-map="clinical">${escapeHtml(t.label)}</button>`
    ).join('');
  ...
  function renderAbnormalMenu(structureId, eye) {
    ...
    html += findings.map((f) =>
      `<button type="button" class="asb-chip" data-asb-add="${structureId}:${eye}:${f.id}">${escapeHtml(f.label)}</button>`
    ).join('');
    html += '</div>';
  ...
  function bindEvents(root) {
    ...
    root.querySelector('#psbSearchInput')?.addEventListener('input', (e) => {
    ...
    root.querySelectorAll('[data-asb-template]').forEach((btn) => {
      btn.addEventListener('click', () => {
        applyTemplate(btn.dataset.asbTemplate);
      });
    });
  ...
  function exportJson() {
    ...
    a.download = `posterior-segment-${$('patientId')?.value || 'export'}.json`;
    a.click();
  ...
  function onKeyDown(e) {
    ...
    if (e.key === '/') {
      e.preventDefault();
      root.querySelector('#psbSearchInput')?.focus();
    }
  }
  ...
  function buildLegacyHiddenFields() {
    const host = $('posteriorSegmentLegacyFields');
    if (!host) return;
    T.STRUCTURES.forEach((struct) => {
      T.EYES.forEach((eye) => {
