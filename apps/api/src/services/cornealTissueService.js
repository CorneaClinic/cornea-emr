...
export function mapCornealTissue(row) {
  return {
    ...
    tissueLaterality: row.tissue_laterality,
    serologyHiv: row.serology_hiv,
    serologyHbv: row.serology_hbv,
    serologyHcv: row.serology_hcv,
    serologySyphilis: row.serology_syphilis,
    serologyCmv: row.serology_cmv,
    serologyClearedAt: row.serology_cleared_at,
    quarantineStatus: row.quarantine_status || 'Cleared',
    quarantineReason: row.quarantine_reason,
    quarantineUntil: formatDate(row.quarantine_until),
...
function parseCornealTissueInput(body) {
  return {
    ...
    donorId: optionalString(body.donorId, 'donorId'),
    lotNumber: optionalString(body.lotNumber, 'lotNumber'),
    tissueLaterality: optionalString(body.tissueLaterality, 'tissueLaterality'),
    serologyHiv: optionalString(body.serologyHiv, 'serologyHiv'),
    serologyHbv: optionalString(body.serologyHbv, 'serologyHbv'),
    serologyHcv: optionalString(body.serologyHcv, 'serologyHcv'),
    serologySyphilis: optionalString(body.serologySyphilis, 'serologySyphilis'),
    serologyCmv: optionalString(body.serologyCmv, 'serologyCmv'),
    quarantineStatus: optionalString(body.quarantineStatus, 'quarantineStatus') || 'Cleared',
    quarantineReason: optionalString(body.quarantineReason, 'quarantineReason'),
    quarantineUntil: parseDate(body.quarantineUntil, 'quarantineUntil'),
    ...
    const { rows } = await query(
      `
        INSERT INTO corneal_tissues (
          ...
          storage_location, eye_bank, donor_id, lot_number, tissue_laterality,
          serology_hiv, serology_hbv, serology_hcv, serology_syphilis, serology_cmv,
          quarantine_status, quarantine_reason, quarantine_until, received_at,
          reserved_kp_patient_id, reserved_for_kp_patient_id, legacy_local_id
        )
        VALUES (
      ...
      [
        ...
        input.donorId,
        input.lotNumber,
        input.tissueLaterality,
        input.serologyHiv,
        input.serologyHbv,
        input.serologyHcv,
        input.serologySyphilis,
        input.serologyCmv,
        input.quarantineStatus,
      ...
      `
        UPDATE corneal_tissues
           SET kp_tissue_id = $3,
               ...
               donor_id = $19,
               lot_number = $20,
               tissue_laterality = $21,
               serology_hiv = $22,
               serology_hbv = $23,
               serology_hcv = $24,
               serology_syphilis = $25,
               serology_cmv = $26,
               quarantine_status = $27,
               quarantine_reason = $28,
               quarantine_until = $29,
               received_at = COALESCE($30::timestamptz, received_at),
               reserved_kp_patient_id = $31,
               reserved_for_kp_patient_id = $32,
               legacy_local_id = $33,
               revision = revision + 1
         WHERE id = $1
           AND clinic_id = $2
